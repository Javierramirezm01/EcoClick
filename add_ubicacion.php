...
<?php
 if(isset($_POST['add_ub'])){
   ...
   if(empty($errors)){
      ...
      $sql .= " VALUES ('{$ub_name}')";
      if($db->query($sql)){
        $session->msg("s", "Ubicacion agregada exitosamente.");
        ...
        <div class="panel-body">
          <form method="post" action="add_ubicacion.php">
            <div class="form-group">
                <input type="text" class="form-control" name="ubicacion" placeholder="Nombre de la Ubicacion" required>
            </div>
            <button type="submit" name="add_ub" class="btn btn-primary">Agregar ubicacion</button>
        </form>
        </div>
      </div>
    </div>
